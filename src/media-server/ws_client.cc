...
void WebSocketClient::init_abr_algo()
{
  ...
  } else if (abr_name_ == "macht_raw") {
    abr_algo_ = make_unique<machtRaw>(*this, abr_name_, abr_config_);
  } else if (abr_name_ == "macht_ttp") {
    abr_algo_ = make_unique<machtTTP>(*this, abr_name_, abr_config_);
  } else if (abr_name_ == "macht_ttp_no_tcp_info") {
    abr_algo_ = make_unique<machtTTP>(*this, abr_name_, abr_config_);
  } else if (abr_name_ == "macht_ttp_mle") {
    abr_algo_ = make_unique<machtTTP>(*this, abr_name_, abr_config_);
