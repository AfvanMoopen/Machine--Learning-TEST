...
static fs::path src_path;  /* path to macht/src directory */
...
int retrieve_expt_id(const string & json_str)
{
  try {
    ...
    create_table.exec("CREATE TABLE IF NOT EXISTS macht_experiment "
    ...
    db_conn.prepare("select_id",
      "SELECT id FROM macht_experiment WHERE hash = $1;");
    db_conn.prepare("insert_json",
      "INSERT INTO macht_experiment (hash, data) VALUES ($1, $2) RETURNING id;");
