#include "macht_ttp.hh"
#include "ws_client.hh"

using namespace std;

machtTTP::machtTTP(const WebSocketClient & client,
                     const string & abr_name, const YAML::Node & abr_config)
  : macht(client, abr_name, abr_config)
{
  ...
  if (abr_config["model_dir"]) {
    ...
    if (abr_name == "macht_ttp_mle") {
      is_mle_= true;
    }

    if (abr_name == "macht_ttp_no_tcp_info") {
      ttp_input_dim_ = 17;
      no_tcp_info_ = true;
    }
    ...
    throw runtime_error("macht requires specifying model_dir in abr_config");
  }
}

void machtTTP::normalize_in_place(size_t i, vector<double> & input)
{
...
void machtTTP::calculate_gaussian_values()
{
...
void machtTTP::blur_probability(int horizontal_index, int format_index)
{
...
void machtTTP::reinit_sending_time()
{
